...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      <div 
        className={`fixed inset-y-0 right-0 z-50 w-full sm:w-96 bg-white shadow-2xl flex flex-col border-l border-gray-100 transition-transform duration-300 ease-in-out transform ${
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div 
                key={item.id} 
                className="flex gap-4 p-3 bg-yellow-50/20 border border-yellow-100/50 rounded-2xl items-center shadow-sm"
              >
                {/* Imagen del producto */}
                <div 
                  className="w-16 h-16 rounded-xl bg-yellow-50 bg-cover bg-center flex-shrink-0"
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center border border-gray-200 rounded-full bg-white shadow-xs">
                      <button 
                        ...
                        className="w-7 h-7 flex items-center justify-center text-gray-500 hover:text-gray-800 text-sm font-semibold rounded-full cursor-pointer select-none"
                      >
                        -
                      </button>
          ...
          <div className="p-5 border-t border-gray-100 bg-white space-y-4">
            ...
            <div className="space-y-2">
              <Link 
                to="/checkout"
                onClick={() => setIsCartOpen(false)}
                className="block w-full text-center py-3 bg-amber hover:bg-amber-600 text-white font-bold rounded-full shadow-md hover:shadow-lg transition-all text-sm active:scale-98"
              >
